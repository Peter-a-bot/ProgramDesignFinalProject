...
static const int dx[] = {0, 0, 1, -1}; // x 軸方向變化
static const int dy[] = {1, -1, 0, 0}; // y 軸方向變化
...
void spawn_monsters(Hero *hero) {
    ...
    for (int i = 0; i < INITIAL_FAST_MONSTER_COUNT; i++) {
        ...
        fast_monsters[i].health = 150;    // 血量較少
        fast_monsters[i].move_chance = 95; // 更高的移動機率 (接近每幀都移動)
        fast_monsters[i].speed_steps = 2; // 一次移動 2 格
...
void move_monsters_towards_player(Hero *hero) {
    for (int i = 0; i < current_fast_monster_count; i++) {
        ...
        bool can_move_to_target = is_valid_monster_position(target_x, target_y);
        ...
        if (can_move_to_target) {
            ...
            if (!collision_with_other_monster) {
                ...
                if (is_valid_monster_position(attempted_x, attempted_y)) {
                ...
                if (!moved_one_step) {
                    ...
                    while (tried < 4) {
                        ...
                        if (is_valid_monster_position(new_x, new_y)) {
    ...
    if (game_fast_monster_time >= next_fast_spawn_time) {
        ...
        for (int i = 0; i < to_add; i++) {
            if (current_fast_monster_count < MAX_FAST_MONSTER_COUNT) {
                random_empty_position(&pos, hero);
                fast_monsters[current_fast_monster_count].x = pos.x;
                fast_monsters[current_fast_monster_count].y = pos.y;
                ...
                fast_monsters[current_fast_monster_count].speed_steps = 2;
        ...
        next_fast_spawn_time = game_fast_monster_time + MONSTER_SPAWN_INTERVAL;
    }
}
    ...
    for (int i = 0; i < current_fast_monster_count; i++) {
        if (fast_monsters[i].is_active) {
            ...
            if ((int)(game_fast_monster_time * 10) % 2 == 0) { // 快速閃爍
                DrawRectangle(fast_monsters[i].y * CELL_SIZE, fast_monsters[i].x * CELL_SIZE, CELL_SIZE * fast_monsters[i].height, CELL_SIZE * fast_monsters[i].width, fast_monsters[i].color);
            } else {
                DrawRectangle(fast_monsters[i].y * CELL_SIZE, fast_monsters[i].x * CELL_SIZE, CELL_SIZE * fast_monsters[i].height, CELL_SIZE * fast_monsters[i].width, DARKBLUE); // 閃爍時變暗
            }
            // 繪製一個黃色邊框來突出快速怪物
            DrawRectangleLinesEx((Rectangle){fast_monsters[i].y * CELL_SIZE, fast_monsters[i].x * CELL_SIZE, CELL_SIZE * fast_monsters[i].height, CELL_SIZE * fast_monsters[i].width}, 1, YELLOW);
        }
    }
}
